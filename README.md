Ниже приведён пример переработанного и улучшенного варианта MD-файла. В этом варианте добавлены оглавление, единый стиль заголовков, форматирование команд и таблиц, а также разбиение на логические блоки для удобства чтения и навигации.
...
1. [Модуль №1: Настройка сетевой инфраструктуры](#модуль-1-настройка-сетевой-инфраструктуры)
   ...
   - [5. Безопасный удалённый доступ](#5-настройка-безопасного-удалённого-доступа)
   - [6. Настройка IP-туннеля между офисами](#6-настройка-ip-туннеля)
   ...
   - [9. DHCP-сервер](#9-настройка-dhcp)
   - [10. DNS-сервер](#10-настройка-dns)
...
2. [Модуль №2: Организация сетевого администрирования ОС](#модуль-2-организация-сетевого-администрирования-ос)
   ...
   - [2. Файловое хранилище](#2-настройка-файлового-хранилища)
   - [3. Служба сетевого времени (chrony)](#3-настройка-chrony)
   - [4. Ansible](#4-настройка-ansible)
   ...
   - [7. Сервис Moodle](#7-настройка-moodle)
   - [8. Обратный прокси на nginx](#8-настройка-nginx-как-обратного-прокси)
   - [9. Установка Яндекс.Браузера](#9-установка-яндексбраузера)

3. [Модуль №3: Эксплуатация объектов сетевой инфраструктуры](#модуль-3-эксплуатация-объектов-сетевой-инфраструктуры)

> **Дополнительно:** Приложения и файлы: файл `users.csv` (отдельный файл).
...
- **Установка и базовая настройка:**
  ...
  nano /etc/named.conf
  ```
  *(Смотрите изображения конфигурации: [named1.png](https://github.com/dizzamer/DEMO2025/blob/main/named1.png), [named2.png](https://github.com/dizzamer/DEMO2025/blob/main/named2.png))*
...
- **Настройка `/etc/nsswitch.conf`:**  
  Проверьте, что сначала используется `/etc/hosts`, затем DNS.
...
### 1. Доменный контроллер Samba на BR-SRV
...
### 5. Развертывание MediaWiki в Docker на BR-SRV
...
### 8. Настройка обратного прокси с nginx на HQ-RTR
...
> **Примечание:** Данный модуль содержит задания по эксплуатации ранее настроенных объектов инфраструктуры.

---

**Необходимые приложения:**
- Файл `users.csv` (отдельный файл).
...
Этот вариант структурирован, использует единый стиль оформления, имеет оглавление для быстрого перехода к нужной секции, а также форматирование команд и таблиц для лучшей читаемости. Вы можете адаптировать и дополнять его согласно требованиям и личным предпочтениям.
